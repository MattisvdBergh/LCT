...
LCGT = function(dataDir, LG, LGS, timePoints = 4,
  ...
  out = getOutput(resultsTemp, Hclass, maxClassSplit1, 
                  ...
                  decreasing = decreasing, levelsDependent = levelsDependent, nIndependent = nIndependent)
...
getOutput = function(resultsTemp, Hclass, maxClassSplit1, 
  ...
  csvTemp = read.csv(paste0("H", Hclass, "c", CC, "_sol", solution, ".csv"),
                     sep = ",", nrows = 2, skip = 7, header = FALSE, dec = ",", na.strings = "")
  ...
  # idx.Parms.time.unordered = matrix(,
  #                                   nrow = levelsDependent - 1 + nIndependent,
  #                                   ncol = solution)
  # counter = 1
  # for(i in 1:solution){
  #   idx.Parms.time.unordered[1:(levelsDependent - 1),i] = counter:(counter + levelsDependent - 2)
  #   counter = (counter + levelsDependent - 1)
  # }
  # counter = max(idx.Parms.time.unordered, na.rm = TRUE) + 1
  # for(i in 1:solution){
  #   idx.Parms.time.unordered[-c(1:(levelsDependent - 1)),i] = seq(counter, counter + (nIndependent - 1) * solution, solution)
  #   counter = (counter + 1)
  # }
  # 
  # Parms.time.unordered = matrix(, ncol = solution, nrow = nrow(idx.Parms.time.unordered), byrow = TRUE)
  # for(i in 1:solution){
  #   Parms.time.unordered[,i] = Parms.temp[-c(1:(solution - 1))][idx.Parms.time.unordered[,i]]
  # }
  ...
  # rownames(Parms.time) = paste0("TimePoint", 1:TimePoints)
