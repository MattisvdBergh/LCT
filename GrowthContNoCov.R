    ...
    for (wPreS in nPreviousSplits) {
      ...
      if (length(namesClasses[[length(namesClasses)]][sizePreviousSplits]) > 1) {
        ...
        for (iCC in seq_along(namesClasses[[length(namesClasses)]][sizePreviousSplits])) {
          ...
          maxRow3 = max(nrow(out[[5]]), nrow(outTemp[[5]]))
          ...
          while(maxRow2 > nrow(outTemp[[5]])){outTemp[[5]] = rbind(outTemp[[5]], NA)}
...
getOutput = function(resultsTemp, Hclass, maxClassSplit1, 
  ...
  lp = length(Parms.tempNoCpp)
  Parms.time.unordered = matrix(Parms.tempNoCpp[-c((lp - solution + 1):lp)], ncol = solution, byrow = TRUE)
  SE.time.unordered = matrix(SE.temp[-c(1:(solution - 1), (lp - solution + 1):lp)], ncol = solution, byrow = TRUE)
  ...
  SE.time = as.matrix(SE.time.unordered[, order.Classpp])
  
  colnames(SE.time) = colnames(Parms.time) = paste0(CC, 1:solution)
