LCT = function(dataDir, LG, LGS,
  ...
  makeSyntaxDic(dataDir, Hclass, syntax, maxClassSplit1)
  ...
  out = getOutputDic(resultsTemp, Hclass, maxClassSplit1, Ntot = Ntot, stopCriterium = stopCriterium, itemNames = itemNames, decreasing = decreasing)
    ...
    for (wPreS in nPreviousSplits) {
      ...
      if (length(namesClasses[[length(namesClasses)]][sizePreviousSplits]) > 1) {
        ...
        for (iCC in seq_along(namesClasses[[length(namesClasses)]][sizePreviousSplits])) {
          ...
          outTemp = getOutputDic(resultsTemp, Hclass, maxClassSplit2, CC = preC[iCC], Ntot = Ntot, stopCriterium = stopCriterium, itemNames = itemNames, decreasing = decreasing)
...
getOutputDic = function(resultsTemp, Hclass, maxClassSplit1, CC = 0, Ntot = Ntot, stopCriterium = stopCriterium, itemNames = itemNames, decreasing = TRUE){
  ...
  EVtemp = matrix(evTemp[seq(2, length(evTemp), 2)],
