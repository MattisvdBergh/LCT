...
#'
#'@export
plotLCTSplitsNoCol = function(resTree,
                         rYlim = c(0, 1),
                         doPlot = FALSE,
                         nameTree = "",
                         dev = "png",
  ...
  for(idxSplits in 1:length(sp)){
    ...
    if(any("meanEV" %in% names(resTree$splitInfo))){
      ...
      for(idxClass in 2:ncol(evSplit)){
        lty = idxClass
        if(lty > 2){lty = lty + 1}
        lines(evSplit[,idxClass], pch = idxClass, type = "l", lty = lty, lwd = lwd)
      }
      axis(1)
      axis(2, las = 2)
    } else{
    ...
    for(idxVar in 2:nrow(evSplit)){
      lines(evSplit[idxVar,], pch = idxVar, type = "b")
    }
  ...
  if(doPlot == TRUE){
    png("Legend.png")
    plot.new()
    legend("center", itemNames, pch = 1:nItems, bty = "n", cex = 2, xpd = TRUE)
    dev.off()
    setwd(mainDir)
  }
