...
LCGT = function(dataDir, LG, LGS, timePoints = 4,
  ...
  out = getOutput(resultsTemp, Hclass, maxClassSplit1, Ntot = Ntot, stopCriterium = stopCriterium, decreasing = decreasing)
    ...
    for (wPreS in nPreviousSplits) {
      ...
      if (length(namesClasses[[length(namesClasses)]][sizePreviousSplits]) > 1) {
        ...
        for (iCC in seq_along(namesClasses[[length(namesClasses)]][sizePreviousSplits])) {
          ...
          outTemp = getOutput(resultsTemp, Hclass, maxClassSplit2, CC = preC[iCC], Ntot = Ntot, stopCriterium = stopCriterium, decreasing = decreasing)
...
getOutput = function(resultsTemp, Hclass, maxClassSplit1, CC = 0, Ntot = Ntot, stopCriterium = stopCriterium, decreasing = TRUE){
  ...
  Parms.temp = csvTemp[1,-c(1:5)][!is.na(csvTemp[1,-c(1:5)])]

  Parms.cpp.temp = c(0, Parms.temp[1:(solution - 1)])

  Parms.time.unordered = matrix(Parms.temp[-c(1:(solution - 1))],
