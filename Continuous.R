LCT = function(dataDir, LG, LGS, decreasing = TRUE, maxClassSplit1 = 2,
               maxClassSplit2 = 2, stopCriterium = "BIC", resultsName = "",
               diffStopCriterium = 0, minSampleSize = 1){

  # Read the original data
  mydata = FullData = read.table(dataDir, header = TRUE)
  Ntot = sum(mydata$weight)
  weight = mydata$weight
  colweight = which(colnames(mydata) == "weight")
  itemNames = colnames(mydata)[1:(colweight - 1)]
  ...
  makeSyntaxCon(dataDir, Hclass, syntax, maxClassSplit1)
...
# Results ####
  ...
  out = getOutputCon(resultsTemp, Hclass, maxClassSplit1, Ntot = Ntot, stopCriterium = stopCriterium, itemNames = itemNames, decreasing = decreasing)
    ...
    for (wPreS in nPreviousSplits) {
      ...
      if (length(namesClasses[[length(namesClasses)]][sizePreviousSplits]) > 1) {
        ...
        ## Make new data ####
          ...
          outTemp = getOutputCon(resultsTemp, Hclass, maxClassSplit2, CC = preC[iCC], Ntot = Ntot, stopCriterium = stopCriterium, itemNames = itemNames, decreasing = decreasing)
          ...
          classSizes = outTemp$Classpp * sum(as.numeric(as.character(outTemp$Post[,1])))
          if(any(classSizes<minSampleSize, na.rm = TRUE)){solution = 1}
...
makeSyntaxCon = function(dataDir, Hclass, syntax, maxClassSplit1, CC = 0) {
  ...
  newSyntax[grep("Cluster nominal", newSyntax)] = paste0("      Cluster nominal ", k, ";")
  write.table(newSyntax, paste0("LCT", CC, ".lgs"), row.names = FALSE, quote = FALSE, col.names = FALSE)
}

getOutputCon = function(resultsTemp, Hclass, maxClassSplit1, CC = 0, Ntot = Ntot, stopCriterium = stopCriterium, itemNames = itemNames, decreasing = TRUE){
  ...
  csvTemp = read.csv(paste0("H", Hclass, "c", CC, "_sol", solution, ".csv"),
                     sep = ";", nrows = 2, skip = 7, header = FALSE, dec = ",")
  ...
  EVtemp = matrix(evTemp,
                  ncol = solution, byrow = TRUE)
  if(solution > 1){
    EV = EVtemp[,order.Classpp]
  } else {
    EV = EVtemp
  }
  
  colnames(EV) = paste0(CC, 1:solution)
  rownames(EV) = itemNames
